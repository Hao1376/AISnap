...
#include "main.h"
/*层的分类 */
typedef enum { Layer1 = 0, Layer2 } LTDC_Layer;
/* RGB LCD部分引脚定义 */
...
void rgblcd_draw_point(LTDC_Layer Layer, uint16_t x, uint16_t y, uint16_t color);                                                             /* RGB LCD画点 */
uint16_t rgblcd_read_point(uint16_t x, uint16_t y);                                                                                           /* RGB LCD读点 */
void rgblcd_overlay_clear(void);                                                                                                              /* 覆盖层清空（全透明） */
void rgblcd_clear(uint16_t color);                                                                                                            /* RGB LCD清屏 */
void rgblcd_draw_line(LTDC_Layer Layer, uint16_t x1, uint16_t y1, uint16_t x2,
                      uint16_t y2, uint16_t color);                                                                                          /* RGB LCD画线 */
void rgblcd_draw_hline(uint16_t x, uint16_t y, uint16_t len, uint16_t color);                                                                /* RGB LCD画水平线 */
void rgblcd_draw_rectangle(LTDC_Layer Layer,uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color);                             /* RGB LCD画矩形 */
void rgblcd_draw_circle(LTDC_Layer Layer,uint16_t x0, uint16_t y0, uint8_t r, uint16_t color);                                               /* RGB LCD画圆 */
void rgblcd_fill_circle(uint16_t x, uint16_t y, uint16_t r, uint16_t color);                                                                 /* RGB LCD画实心圆 */
void rgblcd_show_char(LTDC_Layer Layer,uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color);                        /* RGB LCD在指定位置显示一个字符 */
void rgblcd_show_num(LTDC_Layer Layer,uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color);                      /* RGB LCD显示len个数字 */
void rgblcd_show_xnum(LTDC_Layer Layer, uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color);      /* RGB LCD扩展显示len个数字（显示高位0） */
void rgblcd_show_string(LTDC_Layer Layer, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, char *p, uint16_t color);   /* RGB LCD显示字符串 */
