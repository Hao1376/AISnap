...
/* RGB565 → ARGB1555 颜色转换（bit15=1 不透明，用于 LTDC Layer2 覆盖层） */
#define RGB565_TO_ARGB1555(c) \
    (0x8000U | (((c) & 0xF800U) >> 1) | (((c) & 0x07C0U) >> 1) | ((c) & 0x001FU))
...
/* LTDC帧缓冲区 */
__attribute__((section(".EXTRAM"))) uint16_t g_ltdc_lcd_framebuf[1280 * 800]={0};
__attribute__((section(".EXTRAM"))) uint16_t g_ltdc_overlay_framebuf[1280 * 800]={0};
...
void rgblcd_init(void)
{
    ...
    else if (rgblcddev.id == 0x4384)    /*here ATK-MD0430R-800480 */
    {
        rgblcddev.pwidth = 800;         /* LCD面板的宽度 */
        rgblcddev.pheight = 480;        /* LCD面板的高度 */
    ...
    ltdc_layer_cfg_struct.WindowY0 = 0;
    ltdc_layer_cfg_struct.WindowY1 = rgblcddev.pheight;
    ltdc_layer_cfg_struct.PixelFormat = LTDC_PIXEL_FORMAT_ARGB1555;
    ltdc_layer_cfg_struct.Alpha = 255;
    ltdc_layer_cfg_struct.Alpha0 = 0;
    ltdc_layer_cfg_struct.BlendingFactor1 = LTDC_BLENDING_FACTOR1_PAxCA;
    ltdc_layer_cfg_struct.BlendingFactor2 = LTDC_BLENDING_FACTOR2_PAxCA;
    ltdc_layer_cfg_struct.FBStartAdress = 0;
    ltdc_layer_cfg_struct.ImageWidth = rgblcddev.pwidth;
    ltdc_layer_cfg_struct.ImageHeight = rgblcddev.pheight;
    ltdc_layer_cfg_struct.Backcolor.Blue = 0;
    ltdc_layer_cfg_struct.Backcolor.Green = 0;
    ltdc_layer_cfg_struct.Backcolor.Red = 0;
    HAL_LTDC_ConfigLayer(&hltdc, &ltdc_layer_cfg_struct, 1);
    HAL_LTDC_SetAddress(&hltdc, (uint32_t)g_ltdc_overlay_framebuf, 1);

    /* 手动使能 Layer2（部分 HAL 版本 ConfigLayer 不会自动设 LEN 位） */
    __HAL_LTDC_LAYER_ENABLE(&hltdc, 1);

    /*显示配置 */
    rgblcd_display_dir(0);
    rgblcd_clear(0xFFFF);
    rgblcd_overlay_clear();   /* 覆盖层初始化为全透明 */
    RGBLCD_BL(1);
}
 ...
 * @retval  无
 */
void rgblcd_draw_point(LTDC_Layer Layer,uint16_t x, uint16_t y, uint16_t color)
{
    uint16_t px;
    uint16_t py;
    ...
    if (Layer == Layer1)
    {
        g_ltdc_lcd_framebuf[rgblcddev.pwidth * py + px] = color;                  /* Layer1: RGB565 */
    }
    else
    {
        g_ltdc_overlay_framebuf[rgblcddev.pwidth * py + px] = RGB565_TO_ARGB1555(color); /* Layer2: ARGB1555 */
    }
}
 ...
 * @retval  无
 */
void rgblcd_draw_line(LTDC_Layer Layer,uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    uint16_t t;
    int xerr = 0;
    int yerr = 0;
    ...
    {
        rgblcd_draw_point(Layer,row, col, color);
        xerr += delta_x;
        yerr += delta_y;
        if (xerr > distance)
        {
 ...
 * @retval  无
 */
void rgblcd_draw_rectangle(LTDC_Layer Layer,uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t color)
{
    rgblcd_draw_line(Layer,x1, y1, x2, y1, color);
    rgblcd_draw_line(Layer, x1, y1, x1, y2, color);
    rgblcd_draw_line(Layer, x1, y2, x2, y2, color);
    rgblcd_draw_line(Layer, x2, y1, x2, y2, color);
}
 ...
 * @retval  无
 */
void rgblcd_draw_circle(LTDC_Layer Layer,uint16_t x0, uint16_t y0, uint8_t r, uint16_t color)
{
    int a;
    int b;
    int di;
    ...
    {
        rgblcd_draw_point(Layer,x0 + a, y0 - b, color);
        rgblcd_draw_point(Layer, x0 + b, y0 - a, color);
        rgblcd_draw_point(Layer, x0 + b, y0 + a, color);
        rgblcd_draw_point(Layer,x0 + a, y0 + b, color);
        rgblcd_draw_point(Layer, x0 - a, y0 + b, color);
        rgblcd_draw_point(Layer,x0 - b, y0 + a, color);
 ...
 * @retval  无
 */
void rgblcd_show_char(LTDC_Layer Layer,uint16_t x, uint16_t y, char chr, uint8_t size, uint8_t mode, uint16_t color)
{
    uint8_t csize;
    uint8_t *pfont;
    uint16_t y0;
    ...
    for (t = 0; t < csize; t++)
    {
        ...
        for (t1 = 0; t1 < 8; t1++)                                          /* 遍历一个字节的8个位 */
        {
            ...
            {
                rgblcd_draw_point(Layer,x, y, color);                             /* 以字符颜色绘制这个点 */
            }
            else if (mode == 0)                                             /* 不需要显示的无效点 */
            {
                rgblcd_draw_point(Layer,x, y, g_back_color);                      /* 绘制背景色 */
            }
 ...
 * @retval  无
 */
void rgblcd_show_num(LTDC_Layer Layer,uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint16_t color)
{
    uint8_t t;
    uint8_t temp;
    uint8_t enshow = 0;
    ...
    for (t = 0; t < len; t++)                                                   /* 按总显示位数循环 */
    {
        ...
        if((enshow == 0) && (t < (len - 1)))                                    /* 没有使能显示，且还有位要显示 */
        {
            if (temp == 0)
            {
                rgblcd_show_char(Layer,x + (size >> 1) * t, y, ' ', size, 0, color);  /* 显示空格，占位 */
                continue;                                                       /* 继续下一个位 */
            }
            else
            {
        ...
        rgblcd_show_char(Layer,x + (size >> 1) * t, y, temp + '0', size, 0, color);   /* 显示字符 */
    }
}
 ...
 * @retval  无
 */
void rgblcd_show_xnum(LTDC_Layer Layer, uint16_t x, uint16_t y, uint32_t num, uint8_t len, uint8_t size, uint8_t mode, uint16_t color)
{
    uint8_t t;
    uint8_t temp;
    uint8_t enshow = 0;
    ...
    for (t = 0; t < len; t++)                                                                   /* 按总显示位数循环 */
    {
        ...
        if((enshow == 0) && (t < (len - 1)))                                                    /* 没有使能显示，且还有位要显示 */
        {
            if (temp == 0)
            {
                ...
                {
                    rgblcd_show_char(Layer, x + (size >> 1) * t, y, '0', size, mode & 0x01, color);    /* 显示0，占位 */
                }
                else
                {
                    rgblcd_show_char(Layer, x + (size >> 1) * t, y, ' ', size, mode & 0x01, color);    /* 显示空格，占位 */
                }
                continue;                                                                       /* 继续下一个位 */
            }
            else
        ...
        rgblcd_show_char(Layer, x + (size >> 1) * t, y, temp + '0', size, mode & 0x01, color);        /* 显示字符 */
    }
}
 ...
 * @retval  无
 */
void rgblcd_show_string(LTDC_Layer Layer, uint16_t x, uint16_t y, uint16_t width, uint16_t height, uint8_t size, char *p, uint16_t color)
{
    uint8_t x0;
    ...
    while ((*p <= '~') && (*p >= ' '))              /* 判断是否为非法字符 */
    {
        ...
        rgblcd_show_char(Layer, x, y, *p, size, 0, color); /* 显示一个字符 */
 ...
 */
/**
 * @brief   清空覆盖层（LTDC Layer2），全部像素置为透明
 * @param   无
 * @retval  无
 *
 * ARGB1555 格式下，bit15=0 即透明，因此写入全零 = 全透明。
 * 使用 DMA2D 硬件加速清零。
 */
void rgblcd_overlay_clear(void)
{
    hdma2d.Init.Mode         = DMA2D_R2M;
    hdma2d.Init.OutputOffset = 0;
    HAL_DMA2D_Init(&hdma2d);
    HAL_DMA2D_Start(&hdma2d,
                    0x00000000U,                       /* 全零 = 全透明 ARGB1555 */
                    (uint32_t)g_ltdc_overlay_framebuf,
                    rgblcddev.pwidth,
                    rgblcddev.pheight);
    HAL_DMA2D_PollForTransfer(&hdma2d, 50);
}
